...
/**
 * Print an OP with production-optimized layout.
 */
export async function imprimirOP(opId: string) {
  ...
  const html = `
    ...
    <style>
      ...
      th { background: #f5f5f5; padding: 8px 10px; border: 1px solid #ddd; text-align: left; font-size: 11px; text-transform: uppercase; }
      .obs { margin-top: 16px; padding: 10px; border: 1px dashed #999; font-size: 12px; }
      .footer { margin-top: 30px; font-size: 11px; color: #999; border-top: 1px solid #eee; padding-top: 10px; }
      @media print { body { padding: 10px; } }
    </style></head><body>
    <div class="op-header">
      ...
      <div class="right">
        <div class="field"><span class="label">Código Interno</span><br/><strong style="font-size:18px">${op.numero_op}</strong></div>
    ...
    ${op.observacoes ? `<div class="obs"><strong>Observações:</strong> ${op.observacoes}</div>` : ''}
