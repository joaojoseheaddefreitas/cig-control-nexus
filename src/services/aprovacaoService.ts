  ...
  try {
    ...
    for (const item of dbItens) {
      ...
      for (let f = 0; f < fractions; f++) {
        ...
        const tempoTotal = qtyThisOp * item.tempo_unitario;
        ...
        opsToInsert.push({
          ...
          numero_op: `${codigoPedido}-${String(globalSequence).padStart(2, '0')}`,
          produto_nome: item.produto_nome,
    ...
    if (setoresAtivos && insertedOps) {
      const routeSteps = insertedOps.flatMap((op) =>
        setoresAtivos.map((setor) => ({
          ...
          setor_id: setor.id,
          ordem: setor.ordem,
          tempo_estimado: Number(op.tempo_total) / setoresAtivos.length, // divide evenly as default
        }))
      );
...
export function getOPDisplayMask(
  ...
  if (!totalOps || totalOps <= 1) {
    // Single OP — just the order number (strip the -01 suffix)
    return numeroOp.replace(/-\d+$/, '');
  }
  // Multiple OPs — show sequence/total
  const orderNumber = numeroOp.replace(/-\d+$/, '');
  return `${orderNumber}-${sequenceNumber}/${totalOps}`;
}
