...
const CUSTO_HORA_MOB = 18.5;

export async function fetchCIFData(): Promise<CIFDashboardData> {
  const [pedidosRes, opsRes, materiaisRes, produtosRes, custosFixosRes, funcionariosRes, contasPagarRes, contasReceberRes] = await Promise.all([
    supabase.from("pedidos").select("id, valor_total, status, status_faturamento, created_at").order("created_at", { ascending: false }),
    supabase.from("ops").select("id, produto_nome, quantidade, tempo_unitario, tempo_total, status_producao"),
    (supabase as any).from("materiais").select("estoque_atual, valor_unitario"),
    supabase.from("produtos").select("nome, preco_base, tempo_unitario").eq("ativo", true).limit(20),
    (supabase as any).from("custos_fixos").select("valor_mensal, ativo").eq("ativo", true),
    (supabase as any).from("funcionarios").select("salario, encargos_percentual, tipo_mao_obra, ativo").eq("ativo", true),
    (supabase as any).from("contas_pagar").select("valor, status").eq("status", "pendente"),
    (supabase as any).from("contas_receber").select("valor, status").eq("status", "pendente"),
  ]);
  ...
  const custoMateriais = (materiaisRes.data || []).reduce((s: number, m: any) =>
    s + Number(m.estoque_atual || 0) * Number(m.valor_unitario || 0), 0);

  // Real custos fixos from DB or fallback
  const custosFixosDB = (custosFixosRes.data || []) as any[];
  const custoFixo = custosFixosDB.length > 0
    ? custosFixosDB.reduce((s: number, c: any) => s + Number(c.valor_mensal || 0), 0)
    : 54000;

  // Real folha from DB
  const funcionariosDB = (funcionariosRes.data || []) as any[];
  const folhaPagamento = funcionariosDB.reduce((s: number, f: any) => {
    const sal = Number(f.salario || 0);
    const enc = Number(f.encargos_percentual || 68);
    return s + sal * (1 + enc / 100);
  }, 0);

  const contasPagar = (contasPagarRes.data || []).reduce((s: number, c: any) => s + Number(c.valor || 0), 0);
  const contasReceber = (contasReceberRes.data || []).reduce((s: number, c: any) => s + Number(c.valor || 0), 0);

  const custoTotal = custoMaoDeObra + custoMateriais * 0.3;
  ...
  const rentabilidadeSKU = (produtosRes.data || []).map(p => {
    const custoUnit = Number(p.tempo_unitario) * CUSTO_HORA_MOB * 1.3;
  ...
  allPedidos.forEach(p => {
    ...
    cur.custo += Number(p.valor_total || 0) * 0.7;
    monthMap.set(key, cur);
  });
  const receitaMensal = Array.from(monthMap.entries()).map(([mes, v]) => ({
    mes, receita: v.receita, custo: v.custo,
    margem: v.receita > 0 ? ((v.receita - v.custo) / v.receita) * 100 : 0,
  }));

  return {
    faturamento, custoMateriais, custoMaoDeObra, custoTotal, ebitda, margemLiquida,
    pedidosTotal: allPedidos.length, pedidosFaturados, rentabilidadeSKU,
    receitaMensal: receitaMensal.length > 0 ? receitaMensal : meses.slice(0, 6).map(m => ({ mes: m, receita: 0, custo: 0, margem: 0 })),
    custoFixo, pontoEquilibrio, contasPagar, contasReceber, folhaPagamento,
    funcionariosTotal: funcionariosDB.length,
  };
}
